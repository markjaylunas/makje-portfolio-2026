...
export function LiquidMetalCanvas({
	...
	useEffect(() => {
		function initShader() {
			...
			if (!program || !vertexShader || !fragmentShader) {
				console.error("Failed to create program or shaders");
				return;
			}
	...
	}, [gl, params.speed, imageData, isVisible]);
	...
	useEffect(() => {
		...
		try {
			...
			console.error("Failed to upload image texture");
		}
	...
	useEffect(() => {
		const processImageData = async (data: string) => {
			parseLogoImage(data).then(({ imageData }) => {
				setImageData(imageData);
			});
		};

		processImageData(imageSrc);
	}, []);
		...
		<div className="relative h-full w-full">
			{imageSrcPlaceholder && (
				<img
					...
					aria-hidden="true"
					className={cn(
						"absolute inset-0 block h-full w-full object-contain transition-opacity duration-1000 ease-in-out",
			...
			<canvas
				ref={canvasRef}
				className={cn(
					"relative block h-full w-full object-contain transition-opacity duration-1000 ease-in-out",
